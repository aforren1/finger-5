classdef BlamAudio < superhandle
    ...
    methods
		...
		% if mono_stereo = 'stereo', then stereo
		% if mono_stereo = 'left', then left
		% if = 'right', then right
			...
			elseif strcmpi(mono_stereo, 'left')	|| strcmpi(mono_stereo, 'right')
                if size(snd, 1) > 1
                     warning('Shoving a stereo sound into mono, be aware!');
                     snd = snd(:, 1) + snd(:, 2);
                end
                if strcmpi(mono_stereo, 'left')
                    side = 1;
                else
                    side = 2;
                end
				obj.sound_handle(aud_index) = PsychPortAudio('OpenSlave', obj.pamaster, 1, 1, side);
			else
